            ...
            <div x-show="showSecurityModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
                <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                    ...
                    <div class="inline-block align-bottom bg-studio-surface border border-studio-border rounded-studio text-left overflow-hidden shadow-studio-card transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl w-full p-6 space-y-4">
                        <div class="flex items-start justify-between pb-3 border-b border-studio-border">
                            <div class="flex items-center gap-3">
                                <div class="w-9 h-9 rounded-studio-sm {{ ($criticalCount ?? 0) === 0 ? 'bg-studio-emerald/15 text-studio-emerald' : 'bg-studio-rose/15 text-studio-rose' }} flex items-center justify-center shrink-0">
                            ...
                            <button type="button" @click="showSecurityModal = false" class="text-studio-muted hover:text-white p-1 rounded-studio-sm hover:bg-studio-elevated">
                        ...
                        <div class="max-h-[55vh] overflow-y-auto pr-1 space-y-2.5">
                            ...
                            @if(!empty($issues))
                                @foreach($issues as $key => $detail)
                                    ...
                                    @endphp
                                    <div class="p-3 rounded-studio-sm border border-studio-border bg-studio-bg flex items-start gap-3">
                                        <div class="w-6 h-6 rounded-full flex items-center justify-center shrink-0 mt-0.5
                                            @if($status === 'ok' || $status === 'passed') bg-studio-emerald/15 text-studio-emerald
                                            @elseif($status === 'warning') bg-studio-amber/15 text-studio-amber
                                            @else bg-studio-rose/15 text-studio-rose
                                            @endif">
                                        ...
                                        <div class="min-w-0 flex-1">
                                            <div class="flex items-center justify-between gap-2">
                                                ...
                                                @if($scoreVal !== null)
                                                    <span class="text-[10px] font-mono font-bold text-studio-muted">{{ $scoreVal }}/100</span>
                                                @endif
                                            </div>
                                            <p class="text-xs text-studio-muted mt-0.5">{{ $message }}</p>
                                        </div>
                                    </div>
                                @endforeach
                            @else
                                <div class="text-center py-8 text-xs text-studio-muted">
