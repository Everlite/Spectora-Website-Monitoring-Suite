...
<div class="spectora-card p-4.5 flex flex-col justify-between" 
    ...
    <div>
        ...
        <div class="flex items-start justify-between gap-3 mb-3">
            <div class="min-w-0 flex-1">
                <div class="flex items-center gap-2 mb-1">
                    ...
                    <h3 class="text-xs font-bold text-white truncate hover:text-[#3B57E8] transition-colors">
                        <a href="{{ route('domains.show', $domain) }}">{{ $domain->url }}</a>
                    </h3>
                </div>
                <div class="text-[10px] text-[#8A95A8]">
            ...
            <div class="flex items-center gap-1.5 shrink-0">
                @if($score > 0)
                    <span class="px-2 py-0.5 rounded text-[10px] font-bold font-mono tracking-wider
                        @if($score >= 90) bg-[#10B981]/15 text-[#10B981] border border-[#10B981]/30
                        @elseif($score >= 75) bg-[#3B57E8]/15 text-[#4F6BFF] border border-[#3B57E8]/30
                        @elseif($score >= 50) bg-[#F59E0B]/15 text-[#F59E0B] border border-[#F59E0B]/30
                        @else bg-[#F43F5E]/15 text-[#F43F5E] border border-[#F43F5E]/30
                        @endif">
                        Grade {{ $grade }}
                    </span>
                @endif
                ...
                <button type="button" 
                        ...
                        class="p-1 rounded-studio-sm bg-[#171E2E] hover:bg-[#202A3E] text-[#8A95A8] hover:text-white transition-colors border border-[#202A3E]"
        ...
        <!-- Metrics Inset Grid -->
        <div class="grid grid-cols-2 gap-2 bg-[#090B10] border border-[#202A3E] rounded-studio-sm p-2.5 mb-3">
            <div>
                <div class="text-[10px] uppercase font-bold text-[#8A95A8]">30d Uptime</div>
                <div class="font-mono text-xs font-bold mt-0.5 {{ $uptime >= 99 ? 'text-[#10B981]' : ($uptime >= 95 ? 'text-[#F59E0B]' : 'text-[#F43F5E]') }}">
                    {{ number_format($uptime, 1) }}%
                </div>
            </div>
            <div>
                <div class="text-[10px] uppercase font-bold text-[#8A95A8]">Latenz</div>
            ...
            <div>
                <div class="text-[10px] uppercase font-bold text-[#8A95A8]">SSL Zertifikat</div>
                <div class="font-mono text-[11px] font-medium mt-0.5 {{ $sslDays > 30 ? 'text-white' : ($sslDays > 7 ? 'text-[#F59E0B]' : 'text-[#F43F5E]') }}">
                    {{ $sslDays > 0 ? $sslDays.' d' : ($sslDays === 0 ? 'Expiring' : 'Kein SSL') }}
                </div>
            </div>
            <div>
                <div class="text-[10px] uppercase font-bold text-[#8A95A8]">Besucher (24h)</div>
                <div class="font-mono text-xs font-bold text-white mt-0.5">
    ...
    <!-- Action Footer -->
    <div class="pt-2.5 border-t border-[#202A3E] flex items-center justify-between gap-2">
        ...
        <form method="POST" action="{{ route('domains.destroy', $domain) }}" id="delete-form-{{ $domain->id }}">
            ...
            <button type="button" 
                    ...
                    class="p-1.5 rounded-studio-sm bg-[#171E2E] hover:bg-[#F43F5E]/20 text-[#8A95A8] hover:text-[#F43F5E] transition-colors border border-[#202A3E]" title="Entfernen">
